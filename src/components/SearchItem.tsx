...
export const SearchItem = ({ city, handleClick }: SearchItemProps) => {
  return (
    <Link to='/' onClick={() => handleClick(city.lat, city.lon, city.name)}>
      <a key={city.id} className="p-3 rounded-xl text-slate-100 drop-shadow-lg text-lg backdrop-brightness-95 text-center hover:backdrop-brightness-90 cursor-pointer">
